...
for (i in 1:length(hours)){
    ...
    p[[i]] = ggplot(dfi, aes(x=longitude, y=latitude, color=load)) + 
        geom_point() +
        labs(title = paste("Stations loading - ", load[],"h"))
}
...
for (i in 1:length(hours)){
    ...
    p[[i]] = ggplot(dfi, aes(x=longitude, y=latitude, color=load)) + 
        geom_point() +
        labs(title = paste("Stations loading - ",noms_col[hours[i]],"h"))
}
...
We chose to keep three components so it explains enough the variances (68.6%) but not more to limit the number of components since it does not make us win a lot of explained variances.

```{r}
pca<-PCA(load, scale.unit = T, ncp = 3, graph = F)
```
...
l = list()
for (i in (1:24)){
  l[[i]]=rowMeans(subset(load, select = seq(i, 168, by = 24)))}
load_mean_week <- data.frame(l)
```

```{r}
dim(load_mean_week)
pca2 = PCA(load_mean_week, scale.unit = TRUE,ncp =3, graph=FALSE)
head(pca2$eig)

library(corrplot)
corrplot(pca$var$cor,load_mean_week, is.corr=FALSE, method='ellipse')
```
